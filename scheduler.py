...
class OrchidScheduler:
    ...
    def start_scheduler(self):
        """Start the background scheduler for enrichment"""
        if self.is_running:
            logger.warning("Scheduler is already running")
            return
            
        logger.info("🚀 Starting orchid enrichment scheduler - enrichment enabled")
        self.is_running = True
        
        # Schedule enrichment tasks - AGGRESSIVE schedule for rapid image acquisition
        schedule.every(5).minutes.do(self.update_orchid_records)  # Update records every 5 minutes
        schedule.every(30).minutes.do(self.update_orchid_metadata)  # Metadata refresh every 30 minutes
        schedule.every(30).minutes.do(self.acquire_database_images)  # GBIF/EOL image acquisition every 30 minutes
        ...
        logger.info("✅ Orchid enrichment scheduler started successfully")
    ...
    def acquire_database_images(self):
        """Acquire images from GBIF and EOL databases"""
        logger.info("📸 Running GBIF/EOL image acquisition")
        
        with app.app_context():
            try:
                from gbif_eol_image_acquisition import run_image_acquisition
                
                # Acquire images for 150 orchids per run (AGGRESSIVE MODE - rapid acquisition)
                results = run_image_acquisition(limit=150)
                
                logger.info(f"✅ Image acquisition complete:")
                logger.info(f"   - Processed: {results['total_processed']} orchids")
                logger.info(f"   - GBIF images: {results['gbif_images']}")
                logger.info(f"   - EOL images: {results['eol_images']}")
                logger.info(f"   - Total acquired: {results['total_images_acquired']}")
                
            except Exception as e:
                logger.error(f"❌ Error during image acquisition: {e}")
