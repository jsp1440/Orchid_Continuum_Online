...
try:
    ...
    PIL_AVAILABLE = False
    logger.warning(f"PIL/Pillow not available: {e}. EXIF extraction and image processing will be disabled.")
...
try:
    ...
    logger.warning(f"OpenAI not available: {e}. AI features will be disabled.")

# the newest OpenAI model is "gpt-4o" which was released May 13, 2024.
# do not change this unless explicitly requested by the user
# OpenAI client is now initialized above with graceful fallback
...
def extract_exif_metadata(image_path):
    ...
    if not PIL_AVAILABLE:
        logger.warning("PIL not available - cannot extract EXIF data")
        return {}
