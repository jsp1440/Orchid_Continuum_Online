...
# from orchid_atlas import atlas_bp  # Archived - not needed
...
# from admin_orchid_approval import orchid_approval_bp  # Archived
...
# from data_progress_dashboard import data_dashboard_bp  # Archived
...
import routes_botanist  # Digital Botanist Vision AI routes
import routes_vision_ai  # Vision AI routes
import os
import json
import logging
import requests
...
# import issue_reports  # Archived
# import orchid_match_widget  # Archived - Orchid Match Tinder-style widget
# import article_editor_system  # Archived - Article editor and management
# import widget_access_page  # Archived - Simple widget hub
# import chris_howard_reimport  # Archived
...
Respond helpfully about orchids, weather, navigation, or emergency preparedness. Include relevant commands when appropriate."""
        ...
        # Make API call to OpenAI with production-safe wrapper
        from openai import OpenAI
        from app.ai_utils import safe_ai_call
        
        client = OpenAI(api_key=os.getenv('OPENAI_API_KEY'))
        
        # Wrap OpenAI call with retry + graceful degradation
        ...
        # Check if AI is disabled or failed
        ...
        if ai_result.get("status") == "error":
            return jsonify({
                ...
                'response': f"I apologize, but I'm having technical difficulties right now: {ai_result.get('error')}",
                'commands': []
            })
        
        ai_response = ai_result["result"].choices[0].message.content
...
Based on the current search context, provide helpful guidance for orchid research and search refinement."""
        ...
        # Wrap with production-safe retry logic
        from app.ai_utils import safe_ai_call
        ...
        if ai_result.get("status") == "disabled":
            return jsonify({
                'success': True,
                'response': "🔒 AI search assistance is temporarily paused. Try browsing the gallery or using filter options!",
                'suggested_actions': [],
                'ai_paused': True
            })
        ...
        if ai_result.get("status") == "error":
            return jsonify({
                'success': False,
                'error': str(ai_result.get('error')),
                'response': "I apologize, but AI search assistance is temporarily unavailable. Try using the search filters directly!"
            })
...
def gary_ai_chat():
    ...
    try:
        ...
        # Call OpenAI API with production-safe wrapper
        from app.ai_utils import safe_ai_call
        ...
        if ai_result.get("status") == "disabled":
            return jsonify({
                'success': True,
                'response': "Hi Gary! AI features are temporarily paused while we manage API quotas. Feel free to send a team message instead!",
                'ai_paused': True
            })
...
# DISABLED: Already registered in app.py to prevent duplicate blueprint error
...
if isinstance(globals().get("photo_editor_bp"), Blueprint):
    app.register_blueprint(photo_editor_bp)
else:
    print("⚠️ Skipping photo_editor blueprint: not available")
...
def get_35th_parallel_orchids():
    ...
    try:
        ...
        return parallel_species + [{'display_name': o.display_name or o.scientific_name, 
                                  ...
                                  'region': 'Unknown',
                                  'image_url': f"https://lh3.googleusercontent.com/d/{o.google_drive_id}" if o.google_drive_id else '/static/images/orchid_placeholder.svg'} 
                                 for o in orchids[:10]]
...
def gallery():
    ...
    try:
        ...
        # Build WHERE clause - ONLY show orchids with working Google Drive images
        ...
        # Get the main data - include ALL image fields AND enrichment fields
        main_query = f"""
            SELECT id, display_name, scientific_name, genus, species, author, 
                   ...
                   image_url, image_filename, eol_traits, common_names, eol_page_id, gbif_id
            FROM orchid_record 
            WHERE {where_clause}
            ORDER BY created_at DESC 
            LIMIT %s OFFSET %s
        ...
        class CleanOrchid:
            def __init__(self, row):
                ...
                # Get image from any available source (prioritize in order: google_drive, image_url, image_filename)
                raw_image_url = row[16]
                raw_image_filename = row[17]
                
                if self.google_drive_id:
                    # Use Google's direct image hosting URL for reliable embedding
                    self.image_url = f'https://lh3.googleusercontent.com/d/{self.google_drive_id}'
                elif raw_image_url:
                    self.image_url = raw_image_url
                elif raw_image_filename:
                    self.image_url = f'/static/uploads/{raw_image_filename}'
                else:
                    self.image_url = None
                    
                self.ai_confidence = 0.95
                
                # Enriched metadata from EOL and GBIF
                self.eol_traits = row[18]
                self.common_names = row[19] or ''
                self.eol_page_id = row[20]
                self.gbif_id = row[21]
                
                # Parse enriched metadata for display
                self.enrichment = self._get_enrichment_display()
            ...
            def parse_hybrid_info(self):
                """Extract hybrid name and parentage from display_name"""
                if not self.display_name:
                    return None, None
                
                # Pattern 1: "Genus Name (parent1 x parent2)" - unnamed hybrid with parentage
                match = re.search(r'\(([^)]+\s+x\s+[^)]+)\)', self.display_name)
                if match:
                    parentage = match.group(1).strip()
                    # Get hybrid name (everything before the parentheses, excluding genus)
                    name_part = self.display_name[:match.start()].strip()
                    # Remove genus from the beginning
                    for genus_candidate in [self.genus, self.scientific_name]:
                        if genus_candidate and name_part.startswith(genus_candidate):
                            hybrid_name = name_part[len(genus_candidate):].strip()
                            break
                    else:
                        hybrid_name = name_part
                    
                    # If hybrid_name is generic like "Orchid Seedling L134", treat as unnamed
                    if 'seedling' in hybrid_name.lower() or hybrid_name == '':
                        return None, parentage
                    return hybrid_name if hybrid_name else None, parentage
                
                # Pattern 2: "Genus Name1 x Name2" - hybrid cross without parentheses
                if ' x ' in self.display_name.lower():
                    return None, self.display_name.split(' ', 1)[1] if ' ' in self.display_name else None
                
                return None, None
            
            def get_formatted_name(self):
                """Get formatted display with genus, species, hybrid name, and parentage"""
                parts = []
                
                # Start with genus (italic)
                if self.genus and self.genus.strip() and self.genus not in ['Unknown', '(', 'BOLD:ACX6456']:
                    parts.append(f'<i>{self.genus}</i>')
                
                # Add species if it's a real species name (not 'species', 'hybrid', etc.)
                valid_species = (
                    self.species and 
                    self.species.strip() and 
                    self.species.lower() not in ['species', 'hybrid', 'sp.', 'sp', 'x'] and
                    not self.species.startswith('(')
                )
                
                if valid_species:
                    # Check if species contains hybrid indicator
                    if ' x ' in self.species:
                        parts.append(self.species)
                    else:
                        parts.append(f'<i>{self.species}</i>')
                
                # Parse hybrid information
                hybrid_name, parentage = self.parse_hybrid_info()
                
                # Add hybrid name if present
                if hybrid_name:
                    parts.append(f"'{hybrid_name}'")
                
                # Add parentage if present
                if parentage:
                    parts.append(f'({parentage})')
                
                # If we have nothing useful, fall back to display_name
                if not parts:
                    if self.scientific_name and self.scientific_name.strip():
            ...
            def get_best_name(self):
                """Get the best available name for this orchid (plain text for descriptions)"""
            ...
            def _get_enrichment_display(self):
                """Parse enriched metadata for gallery display"""
                enrichment = {
                    'has_data': False,
                    ...
                    'sample_locations': [],
                    'has_eol': bool(self.eol_page_id),
                    'has_gbif': bool(self.gbif_id)
                }
                
                try:
                    if self.eol_traits:
                        import json
                        traits = json.loads(self.eol_traits)
                        enrichment['has_data'] = True
                        
                        # EOL description
                        if traits.get('eol_description'):
                            enrichment['eol_description'] = traits['eol_description'][:400]
                        
                        # Common names
                        if traits.get('common_names'):
                            names = [cn['name'] for cn in traits['common_names'][:2]]
                            enrichment['common_names_list'] = names
                        
                        # GBIF conservation and occurrences
                        if traits.get('gbif_conservation'):
                            enrichment['conservation_status'] = traits['gbif_conservation'].get('status_indicator')
                            enrichment['occurrence_count'] = traits['gbif_conservation'].get('occurrence_count')
                        
                        # Sample locations
                        if traits.get('gbif_occurrences'):
                            locs = traits['gbif_occurrences'].get('sample_locations', [])
                            enrichment['sample_locations'] = locs[:3]
                    
                    # Fallback to common_names field
                    if not enrichment['common_names_list'] and self.common_names:
                        enrichment['common_names_list'] = [n.strip() for n in self.common_names.split(',')[:2]]
                    
                except Exception as e:
                    logger.debug(f"Error parsing enrichment for orchid {self.id}: {e}")
        ...
        try:
            ...
            class DirectOrchid:
                def __init__(self, row):
                    ...
                    # Computed properties
                    self.image_url = f'https://lh3.googleusercontent.com/d/{self.google_drive_id}' if self.google_drive_id else None
                    self.ai_confidence = 0.95
...
@app.route('/admin/metadata-scraper')
def admin_metadata_scraper_dashboard():
    """Dashboard for metadata-only scraping (no image downloads)"""
    try:
        # Get current statistics
        stats = {
            'total_orchids': OrchidRecord.query.count(),
            'with_image_url': OrchidRecord.query.filter(OrchidRecord.image_url != None).count(),
            'missing_metadata': OrchidRecord.query.filter(
        ...
        return render_template('admin_metadata_scraper.html', 
                             stats=stats,
                             recent_logs=recent_logs)
    except Exception as e:
        logger.error(f"Error loading metadata scraper dashboard: {e}")
        return render_template('admin_metadata_scraper.html',
                             stats={'total_orchids': 0, 'with_image_url': 0, 'missing_metadata': 0},
                             recent_logs=[])

@app.route('/admin/run-metadata-scraper', methods=['POST'])
@csrf.exempt
def admin_run_metadata_scraper():
    """Run the metadata-only scraper"""
    try:
        data = request.get_json() or {}
        source = data.get('source', 'both')  # gary, roberta, or both
        max_images = int(data.get('max_images', 50))
        
        logger.info(f"Starting metadata-only scraper: source={source}, max_images={max_images}")
        
        # Import and run the metadata-only scraper
        from metadata_only_scraper import MetadataOnlyScraper
        
        scraper = MetadataOnlyScraper()
        results = scraper.run_metadata_enrichment(source=source, max_images=max_images)
        
        return jsonify({
            'success': True,
            'urls_collected': results['urls_collected'],
            'images_processed': results['images_processed'],
            'orchids_saved': results['orchids_saved'],
            'failed': results.get('failed', 0),
            'errors': results.get('errors', [])
        })
        
    except Exception as e:
        logger.error(f"Error running metadata-only scraper: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({
            'success': False,
...
# NOTE: Enrichment monitor routes moved to after admin_required import (line 6674+)
...
@app.route('/admin/enrichment-monitor')
@admin_required
def admin_enrichment_monitor():
    """Real-time enrichment monitoring dashboard"""
    return render_template('admin/enrichment_monitor.html')

@app.route('/admin/enrichment/api/stats')
@admin_required
def admin_enrichment_stats():
    ...
    try:
        ...
        if progress_file.exists():
            ...
            if data.get('start_time'):
                ...
                rate = data['processed'] / elapsed * 60 if elapsed > 0 else 0
                remaining = (data['total'] - data['processed']) / (rate/60) if rate > 0 else 0
                
                data['rate'] = rate
                data['eta'] = f"{remaining/60:.1f} minutes" if remaining > 0 else "Complete"
                data['status'] = 'running' if is_running else ('complete' if data['processed'] >= data['total'] else 'stopped')
            
            return jsonify(data)
        else:
            return jsonify({'status': 'not_started', 'total': 0, 'processed': 0, 'enriched': 0, 'images_downloaded': 0})
    except Exception as e:
        logger.error(f"Error getting enrichment stats: {e}")
        return jsonify({'error': str(e)}), 500

@app.route('/admin/enrichment/api/logs')
@admin_required
def admin_enrichment_logs():
    """Get enrichment log contents"""
    from pathlib import Path
    
    try:
        log_file = Path('enrichment_full.log')
        if log_file.exists():
...
@app.route('/admin/enrichment/api/summary')
@admin_required
def admin_enrichment_summary():
    """Get enrichment summary"""
    import json
    from pathlib import Path
    
    try:
        summary_file = Path('enrichment_summary.json')
        if summary_file.exists():
            with open(summary_file) as f:
                data = json.load(f)
            
            if data.get('duration_minutes'):
                data['rate'] = data['processed'] / data['duration_minutes']
...
def plural_gallery_widget():
    ...
    try:
        ...
        genus_filter = request.args.get('genus', None)
        page = request.args.get('page', 1, type=int)
        
        # Get all distinct genera for dropdown
        genera_query = db.session.query(
            OrchidRecord.genus, 
            func.count(OrchidRecord.id).label('count')
        ).filter(
            OrchidRecord.genus.isnot(None),
            OrchidRecord.google_drive_id.isnot(None),
            OrchidRecord.validation_status != 'rejected'
        ).group_by(OrchidRecord.genus).order_by(func.count(OrchidRecord.id).desc())
        
        all_genera = [{'genus': g.genus, 'count': g.count} for g in genera_query.all()]
        
        # Get orchids based on filter
        query = OrchidRecord.query.filter(
            OrchidRecord.google_drive_id.isnot(None),
            OrchidRecord.validation_status != 'rejected'
        )
        ...
        # Get total count for selected genus
        total_count = query.count()
        
        # Paginate results
        orchids = query.limit(limit).offset((page - 1) * limit).all()
        ...
        for orchid in orchids:
            orchids_data.append({
                ...
                'species': orchid.species,
                'ai_description': orchid.ai_description,
                'image_url': f"https://lh3.googleusercontent.com/d/{orchid.google_drive_id}" if orchid.google_drive_id else None
            })
        ...
        # Emergency fallback
        return render_template('widgets/gallery_widget.html', 
                             widget_data={'orchids': [], 'total_count': 0, 'genera': []}, 
                             standalone=True)
        ...
        fallback_orchids = [
            {
                ...
                "image_url": f"https://lh3.googleusercontent.com/d/185MlwyxBU8Dy6bqGdwXXPeBXTlhg5M0I"
            },
            {
                "id": 1391,
                ...
                "decimal_latitude": None,
                "decimal_longitude": None,
                "image_url": f"https://lh3.googleusercontent.com/d/1142ajwZe7_LbGt-BPy-HqVkLpNczcfZY"
            },
        {
            "id": 1003,
            ...
            "image_url": f"https://lh3.googleusercontent.com/d/1bUDCfCrZCLeRWgDrDQfLbDbOmXTDQHjH"
        },
        {
            "id": 1004,
            ...
            "image_url": f"https://lh3.googleusercontent.com/d/1gd9BbXslt1IzAgMpeMWYQUfcJHWtHzhS"
        }
    ]
...
# DISABLED FOR STABILITY: Background monitoring threads causing crashes
try:
    # register_admin_control_center(app)  # DISABLED - starts comprehensive_system_monitor
    logger.info("📊 Admin Control Center DISABLED for stability")
except Exception as e:
...
# Register Orchid Data Enrichment System
# DISABLED: Already registered in app.py to prevent duplicate blueprint error
# try:
#     from orchid_data_enrichment import orchid_enrichment
#     app.register_blueprint(orchid_enrichment)
#     logger.info("🌺 Orchid Data Enrichment System registered successfully")
# except Exception as e:
#     logger.error(f"Failed to register enrichment system: {e}")

# Start automated repair system
# DISABLED FOR STABILITY: Background threads causing resource exhaustion and crashes
try:
    # if repair_system:
    #     repair_system.start_repair_system()
    logger.info("🔧 Automated Repair System DISABLED for stability")
except Exception as e:
    logger.error(f"Failed to start repair system: {e}")

# Register Quick Enrichment system
try:
    from admin_quick_enrichment import quick_enrich_bp
    app.register_blueprint(quick_enrich_bp)
    logger.info("⚡ Quick Enrichment system registered successfully")
except Exception as e:
    logger.error(f"Failed to register quick enrichment: {e}")

# Register Automated Batch Enrichment system
try:
    from automated_batch_enrichment import auto_enrich_bp
    app.register_blueprint(auto_enrich_bp)
    logger.info("🤖 Automated Batch Enrichment system registered successfully")
except Exception as e:
    logger.error(f"Failed to register automated enrichment: {e}")

# Register Enrichment Status Dashboard
try:
    from enrichment_status_dashboard import enrichment_status_bp
    app.register_blueprint(enrichment_status_bp)
    logger.info("📊 Enrichment Status Dashboard registered successfully")
except Exception as e:
    logger.error(f"Failed to register enrichment status dashboard: {e}")

# Register Field Completion Dashboard
try:
    from field_completion_routes import field_completion_bp
    app.register_blueprint(field_completion_bp)
    logger.info("📈 Field Completion Dashboard registered successfully")
except Exception as e:
    logger.error(f"Failed to register field completion dashboard: {e}")

# Register Manual Image Acquisition Trigger
try:
    from manual_acquisition_trigger import manual_acquisition_bp
    app.register_blueprint(manual_acquisition_bp)
    logger.info("⚡ Manual Image Acquisition Trigger registered successfully")
except Exception as e:
    logger.error(f"Failed to register manual acquisition trigger: {e}")

# Register Massive Orchid Scraper
try:
    from massive_scrape_routes import massive_scrape_bp
    app.register_blueprint(massive_scrape_bp)
    logger.info("🌊 MASSIVE ORCHID SCRAPER registered successfully - Target: 100K+ images")
except Exception as e:
    logger.error(f"Failed to register massive scraper: {e}")

# Register Simple Live Scraper (shows actual images/data)
try:
    from simple_live_scraper import simple_scraper_bp
    app.register_blueprint(simple_scraper_bp)
    logger.info("📺 LIVE SCRAPER registered - Shows actual images and data in real-time")
except Exception as e:
    logger.error(f"Failed to register live scraper: {e}")

# Register Image Validation System
try:
    from image_validation_system import validation_bp
    app.register_blueprint(validation_bp)
    logger.info("🚨 IMAGE VALIDATION SYSTEM registered - Users can now report incorrect IDs")
except Exception as e:
    logger.error(f"Failed to register validation system: {e}")

# Register Orchid Verification System
try:
    from orchid_verification_system import verification_bp
    app.register_blueprint(verification_bp)
    logger.info("✅ ORCHID VERIFICATION SYSTEM registered - Systematic GBIF validation for all orchids")
except Exception as e:
    logger.error(f"Failed to register verification system: {e}")

# Register Validation Admin Routes
try:
    from validation_admin_routes import validation_admin_bp
    app.register_blueprint(validation_admin_bp)
    logger.info("🔬 VALIDATION ADMIN registered - Image validation review dashboard at /admin/validation/review")
except Exception as e:
    logger.error(f"Failed to register validation admin: {e}")
...
from article_routes import articles_bp
...
@app.route('/widgets/orchid-of-day')
def widget_orchid_of_day():
    """Orchid of the Day Widget"""
    try:
        from utils import get_orchid_of_day
        orchid_data = get_orchid_of_day()
        return render_template('widgets/orchid_of_day.html', orchid=orchid_data, standalone=True)
    except:
        orchid = OrchidRecord.query.filter(OrchidRecord.image_url.isnot(None)).order_by(db.func.random()).first()
        return render_template('widgets/orchid_of_day.html', orchid=orchid, standalone=True)

@app.route('/widgets/recent-orchids')
def widget_recent_orchids():
    """Recent Orchids Widget"""
    recent = OrchidRecord.query.order_by(OrchidRecord.created_at.desc()).limit(8).all()
    return render_template('widgets/discovery_widget.html', widget_data={'orchids': recent}, standalone=True)

@app.route('/widgets/gallery-featured')
def widget_gallery_featured():
    """Featured Gallery Widget"""
    featured = OrchidRecord.query.filter_by(is_featured=True).limit(12).all()
    return render_template('widgets/gallery_widget.html', widget_data={'orchids': featured}, standalone=True)

@app.route('/widgets/search-preview')
def widget_search_preview():
    """Search Preview Widget"""
    try:
        genera = db.session.query(OrchidRecord.genus).distinct().limit(20).all()
        genera_list = [g[0] for g in genera if g[0]]
        return render_template('widgets/search_widget.html', genera=genera_list, standalone=True)
    except:
        return render_template('widgets/search_widget.html', genera=[], standalone=True)

@app.route('/widgets/weather-habitat')
def widget_weather_habitat():
    """Weather Habitat Comparison Widget"""
    orchid = OrchidRecord.query.filter(OrchidRecord.image_url.isnot(None)).first()
    widget_data = {'orchid': orchid} if orchid else {}
    return render_template('widgets/climate_widget.html', widget_data=widget_data, standalone=True)
    ...
    try:
        ...
        for zone in climate_zones:
            ...
            orchids_by_climate[zone] = [{
                ...
                'scientific_name': orchid.scientific_name,
                'image_url': f'https://lh3.googleusercontent.com/d/{orchid.google_drive_id}' if orchid.google_drive_id else '/static/images/orchid_placeholder.svg',
...
# app.register_blueprint(data_dashboard_bp)  # Archived
...
def unidentified_orchids():
    ...
    try:
        ...
        for orchid in unidentified:
            if orchid.google_drive_id:
                orchid.image_url = f'https://lh3.googleusercontent.com/d/{orchid.google_drive_id}'
            else:
...
@app.route('/widgets')
@app.route('/widget-directory')
def widget_directory():
    """Widget Directory - All available interactive tools and widgets"""
    return render_template('widget_directory.html')
...
# Import Master AI Widget Manager (ONLY if AI is enabled)
...
if ORCHID_AI_ENABLED:
    ...
    logger.info("⏸️  Master AI Widget Manager DISABLED (ORCHID_AI_ENABLED=false)")
...
def eol_orchid_explorer(orchid_id=None):
    ...
    try:
        ...
        if orchid_id:
            ...
            # Skip EOL data to prevent timeouts - return None
            eol_data = None
            ...
            for orchid in enhanced_orchids:
                # Skip EOL API calls - just return orchid data to prevent timeouts
                orchid_data.append({
                    'id': orchid.id,
                    'scientific_name': orchid.scientific_name,
                    'display_name': orchid.display_name,
...
# ========================================
# ADMIN NAME VALIDATION ROUTES
# ========================================

@app.route('/admin/name-validation')
def admin_name_validation():
    """Admin dashboard for validating and correcting orchid names"""
    try:
        page = request.args.get('page', 1, type=int)
        per_page = 12
        genus_filter = request.args.get('genus', '')
        
        # Base query for records needing validation
        query = OrchidRecord.query
        
        if genus_filter:
            query = query.filter_by(genus=genus_filter)
        
        # Filter for records with generic species names
        query = query.filter(
            or_(
        ...
        # Get paginated results
        pagination = query.order_by(OrchidRecord.id).paginate(
            page=page, 
            per_page=per_page, 
            error_out=False
        )
        ...
        # Get statistics
        trichocentrum_count = OrchidRecord.query.filter_by(
            genus='Trichocentrum', 
            species='species'
        ).count()
        
        needs_species_count = OrchidRecord.query.filter(
            OrchidRecord.species == 'species'
        ).count()
        
        generic_hybrid_count = OrchidRecord.query.filter(
            OrchidRecord.species == 'hybrid'
        ).count()
        
        return render_template('admin/name_validation.html',
                             orchids=pagination.items,
                             page=page,
                             total_pages=pagination.pages,
                             trichocentrum_count=trichocentrum_count,
                             needs_species_count=needs_species_count,
                             generic_hybrid_count=generic_hybrid_count,
                             genus_filter=genus_filter)
        
    except Exception as e:
        logger.error(f"Error in admin name validation: {e}")
        flash(f'Error loading validation dashboard: {str(e)}', 'danger')
        return redirect(url_for('admin_dashboard'))


@app.route('/admin/update-orchid/<int:orchid_id>', methods=['POST'])
def admin_update_orchid(orchid_id):
    """Update orchid identification"""
    try:
        data = request.json
        orchid = OrchidRecord.query.get_or_404(orchid_id)
        ...
        # Update fields
        if 'genus' in data:
            orchid.genus = data['genus'].strip()
        if 'species' in data:
            orchid.species = data['species'].strip()
        if 'display_name' in data:
            orchid.display_name = data['display_name'].strip()
        
        # Update scientific name
        if orchid.genus and orchid.species:
            orchid.scientific_name = f"{orchid.genus} {orchid.species}"
        ...
        logger.error(f"Error updating orchid {orchid_id}: {e}")
        db.session.rollback()
        return jsonify({
            'success': False,
            'error': str(e)
...
@app.route('/admin/ai-identify/<int:orchid_id>', methods=['POST'])
def admin_ai_identify(orchid_id):
    """Use AI to identify orchid from image"""
    try:
        orchid = OrchidRecord.query.get_or_404(orchid_id)
        ...
        if not image_url:
            return jsonify({
                'success': False,
                'error': 'No image available for this orchid'
            }), 400
        ...
        if ai_result and 'genus' in ai_result:
            return jsonify({
                'success': True,
                'genus': ai_result.get('genus', ''),
                'species': ai_result.get('species', ''),
                'display_name': ai_result.get('scientific_name', ''),
                'confidence': ai_result.get('confidence', 0)
            })
        else:
            return jsonify({
                'success': False,
                'error': 'AI could not identify this orchid'
            }), 400
            
    except Exception as e:
        logger.error(f"Error with AI identification for orchid {orchid_id}: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
logger.info("🔍 Admin Name Validation routes registered at /admin/name-validation")

# ========================================
# IMAGE COLLECTION SYSTEM
# ========================================
try:
    from image_collection_routes import image_collection_bp
    app.register_blueprint(image_collection_bp)
    logger.info("🖼️ Image Collection System registered successfully")
except ImportError as e:
    logger.warning(f"Image Collection System not available: {e}")

# ========================================
# ETHNOBOTANY WIDGET SYSTEM
# ========================================
try:
    from routes_ethnobotany_widget import ethnobotany_widget_bp
    app.register_blueprint(ethnobotany_widget_bp)
    logger.info("🌿 Ethnobotany Widget System registered successfully")
except ImportError as e:
    logger.warning(f"Ethnobotany Widget System not available: {e}")

# ========================================
# MEDICINAL ORCHID MAP WIDGET
# ========================================
try:
    from routes_medicinal_map import medicinal_map_bp
    app.register_blueprint(medicinal_map_bp)
    logger.info("🗺️ Medicinal Orchid Map Widget registered successfully")
except ImportError as e:
    logger.warning(f"Medicinal Orchid Map Widget not available: {e}")

# ========================================
# ETHNOBOTANY TIMELINE WIDGET
# ========================================
try:
    from routes_ethnobotany_timeline import ethnobotany_timeline_bp
    app.register_blueprint(ethnobotany_timeline_bp)
    logger.info("📅 Ethnobotany Timeline Widget registered successfully")
except ImportError as e:
    logger.warning(f"Ethnobotany Timeline Widget not available: {e}")

# ========================================
# ETHNOBOTANY WIDGET PACKAGE (Neon One Embeds)
# ========================================
try:
    from ethnobotany_widget_package import ethnobotany_package_bp
    app.register_blueprint(ethnobotany_package_bp)
    logger.info("📦 Ethnobotany Widget Package registered for Neon One deployment")
except ImportError as e:
    logger.warning(f"Ethnobotany Widget Package not available: {e}")

# ========================================
# AUTONOMOUS IMAGE ACQUISITION DASHBOARD
# ========================================
try:
    from routes_autonomous_dashboard import autonomous_dashboard_bp
    app.register_blueprint(autonomous_dashboard_bp)
    logger.info("🤖 Autonomous Image Acquisition Dashboard registered successfully")
except ImportError as e:
    logger.warning(f"Autonomous Dashboard not available: {e}")


@app.route('/newsletters')
def newsletter_archive():
    """Public newsletter archive page for Neon One embedding"""
    # Generate sample newsletter data (replace with actual data from your system)
    newsletters = [
        {
            'month': 'October',
            'year': 2025,
            'title': 'Fall Orchid Care & Member Showcase',
            'preview_text': 'Featured: Member growing tips for fall blooming orchids, photo contest winners, and upcoming events',
            'pdf_url': '/static/newsletters/2025-10.pdf',
            'web_url': '/newsletters/2025/october',
            'featured_photos': 12
        },
        {
            'month': 'September',
            'year': 2025,
            'title': 'Show Season Highlights',
            'preview_text': 'Recap of our annual orchid show with award-winning specimens and member achievements',
            'pdf_url': '/static/newsletters/2025-09.pdf',
            'web_url': '/newsletters/2025/september',
            'featured_photos': 18
        },
        {
            'month': 'August',
            'year': 2025,
            'title': 'Summer Growing Guide',
            'preview_text': 'Expert tips for summer orchid care, humidity management, and pest prevention',
            'pdf_url': '/static/newsletters/2025-08.pdf',
            'web_url': '/newsletters/2025/august',
            'featured_photos': 10
        },
        {
            'month': 'July',
            ...
            'preview_text': 'Meet our new members, beginner growing guide, and monthly orchid spotlight',
            'pdf_url': '/static/newsletters/2025-07.pdf',
            'web_url': '/newsletters/2025/july',
            'featured_photos': 8
        },
        {
            'month': 'June',
            'year': 2025,
            'title': 'Outdoor Growing Season',
            'preview_text': 'Transitioning orchids outdoors, summer care calendar, and member Q&A',
            'pdf_url': '/static/newsletters/2025-06.pdf',
            'web_url': '/newsletters/2025/june',
            'featured_photos': 15
        },
        {
            'month': 'May',
            ...
            'preview_text': 'Spring flowering orchids in full bloom, repotting guide, and upcoming workshops',
            'pdf_url': '/static/newsletters/2025-05.pdf',
            'web_url': '/newsletters/2025/may',
            'featured_photos': 20
        }
    ]
    
    return render_template('newsletters/archive.html', newsletters=newsletters)


@app.route('/widgets/directory')
@app.route('/all-widgets')
def complete_widget_directory():
    """Complete directory of all widgets in one organized page"""
    return render_template('complete_widget_directory.html')


@app.route('/orchid/<int:orchid_id>/details')
def orchid_detail_view(orchid_id):
    """Full orchid detail page with all enriched data"""
    orchid = OrchidRecord.query.get_or_404(orchid_id)
    
    # Prepare image URL
    orchid.image_url = f"https://lh3.googleusercontent.com/d/{orchid.google_drive_id}" if orchid.google_drive_id else None
    
    return render_template('orchid_detail_modal.html', orchid=orchid)
...
# ========================================
try:
    from orchid_image_report_routes import orchid_report_bp
    app.register_blueprint(orchid_report_bp)
    logger.info("🚨 Orchid Image Reporting System registered successfully")
except ImportError as e:
    logger.warning(f"Orchid Image Reporting System not available: {e}")
