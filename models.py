...
class OrchidTaxonomy(db.Model):
    ...
    # GBIF/EOL Research Integration - Complete Taxonomic Hierarchy
    kingdom = db.Column(String(120), nullable=True)
    phylum = db.Column(String(120), nullable=True)
    class_ = db.Column('class', String(120), nullable=True)  # 'class' is SQL keyword, aliased
    order = db.Column(String(120), nullable=True)
    family = db.Column(String(120), nullable=True, index=True)
    subfamily = db.Column(String(120), nullable=True, index=True)  # Orchid subfamily (e.g., Epidendroideae)
    tribe = db.Column(String(120), nullable=True, index=True)      # Orchid tribe (e.g., Vandeae)
    subtribe = db.Column(String(120), nullable=True, index=True)   # Orchid subtribe (e.g., Aeridinae)
    # Note: genus and species defined above (lines 534-535)
    subspecies = db.Column(String(120), nullable=True)
    variety = db.Column(String(120), nullable=True)
    
    # Taxonomic metadata
    taxon_rank = db.Column(String(50), nullable=True)  # species, subspecies, variety, etc.
    taxonomic_status = db.Column(String(50), nullable=True)  # ACCEPTED, SYNONYM, DOUBTFUL
    
    # External database integration
    gbif_taxon_key = db.Column(db.BigInteger, nullable=True, index=True)  # GBIF unique species identifier (BIGINT)
    eol_page_id = db.Column(String(32), nullable=True, index=True)  # Encyclopedia of Life page ID
    gbif_occurrence_count = db.Column(Integer, default=0)  # Number of wild observations
    external_ids = db.Column(JSON, nullable=True)  # Other external identifiers (JSONB)
    
    # Multilingual support
    vernacular_names = db.Column(JSON, nullable=True)  # JSONB array of vernacular names with languages
    synonyms_json = db.Column(JSON, nullable=True)  # JSONB array of scientific synonyms
    
    # Sync timestamps
    gbif_last_synced_at = db.Column(DateTime, nullable=True)
    eol_last_synced_at = db.Column(DateTime, nullable=True)
    last_taxonomic_update = db.Column(DateTime, nullable=True)
...
class OrchidRecord(db.Model):
    ...
    # Distribution map (from GBIF occurrence data)
    distribution_map_html = db.Column(Text, nullable=True)  # Folium interactive map HTML
    ...
    image_attribution = db.Column(Text, nullable=True)  # Attribution for image source
    ...
    gbif_occurrence_key = db.Column(db.BigInteger, nullable=True, index=True)  # GBIF occurrence key (BIGINT)
    gbif_species_key = db.Column(Integer, nullable=True)  # GBIF species key
    gbif_dataset_key = db.Column(String(64), nullable=True)  # GBIF dataset key
    ...
    # GBIF Collection/Specimen Metadata (Research Integration)
    institution_code = db.Column(String(100), nullable=True)  # Museum/herbarium code
    collection_code = db.Column(String(100), nullable=True)  # Collection within institution
    catalog_number = db.Column(String(150), nullable=True)  # Specimen catalog number
    recorded_by = db.Column(String(200), nullable=True)  # Collector name
    record_number = db.Column(String(120), nullable=True)  # Field collection number
    basis_of_record = db.Column(String(40), nullable=True)  # PRESERVED_SPECIMEN, OBSERVATION, etc.
    gbif_publishing_org_key = db.Column(String(64), nullable=True)  # Who published the data
    
    # Geographic Precision (Research Integration)
    latitude = db.Column(db.Numeric, nullable=True)  # High-precision latitude (NUMERIC)
    longitude = db.Column(db.Numeric, nullable=True)  # High-precision longitude (NUMERIC)
    elevation_m = db.Column(db.Numeric, nullable=True)  # Altitude in meters (NUMERIC)
    coordinate_uncertainty_m = db.Column(db.Numeric, nullable=True)  # GPS uncertainty (NUMERIC)
    geospatial_quality = db.Column(String(20), nullable=True)  # verified, estimated, low_confidence, unknown
    
    # Observation & Collection Metadata (Research Integration)
    observation_date = db.Column(DateTime, nullable=True)  # When observed/collected
    identification_date = db.Column(DateTime, nullable=True)  # When identified
    identified_by = db.Column(String(200), nullable=True)  # Who identified it
    source_dataset = db.Column(String(200), nullable=True)  # Which dataset provided the data
    record_verification_status = db.Column(String(50), nullable=True)  # Verification status
    revision_notes = db.Column(Text, nullable=True)  # Taxonomic revision notes
    
    # Voucher/Specimen Information (Research Integration)
    voucher_type = db.Column(String(50), nullable=True)  # Type of voucher
    voucher_institution = db.Column(String(200), nullable=True)  # Institution holding voucher
    voucher_catalog_number = db.Column(String(150), nullable=True)  # Voucher catalog number
    location_protocol = db.Column(String(200), nullable=True)  # How location was determined
    
    # Plant Morphology Measurements (Research Integration)
    leaf_count = db.Column(Integer, nullable=True)  # Number of leaves
    plant_height_mm = db.Column(db.Numeric, nullable=True)  # Plant height in millimeters (NUMERIC)
    pseudobulb_count = db.Column(Integer, nullable=True)  # Number of pseudobulbs
    pseudobulb_size_mm = db.Column(db.Numeric, nullable=True)  # Pseudobulb size (NUMERIC)
    
    # Pollination Observations (Research Integration)
    pollination_observed = db.Column(Boolean, nullable=True)  # Was pollination observed
    pollinator_observed_id = db.Column(Integer, nullable=True)  # Reference to pollinator
    
    # Cultivation Context (Research Integration)
    record_context = db.Column(String(100), nullable=True)  # Context of record
    cultivar_name = db.Column(String(200), nullable=True)  # Cultivar name
    growing_medium = db.Column(String(100), nullable=True)  # Growing medium
    container_type = db.Column(String(100), nullable=True)  # Container type
    exposure = db.Column(String(100), nullable=True)  # Light exposure
    
    # Data Licensing & Privacy (Research Integration)
    data_license = db.Column(String(100), nullable=True)  # Data license
    sensitive_flag = db.Column(Boolean, default=False)  # Is data sensitive
    public_display_location = db.Column(String(200), nullable=True)  # Public display location
    
    # Phenology Tracking (Research Integration)
    bloom_start_month = db.Column(Integer, nullable=True)  # Month bloom starts (1-12)
    bloom_end_month = db.Column(Integer, nullable=True)  # Month bloom ends (1-12)
    bloom_intensity = db.Column(db.Numeric, nullable=True)  # Bloom intensity score (NUMERIC)
    
    # Asset Management (Research Integration)
    asset_id = db.Column(Integer, nullable=True)  # Asset ID reference
    ...
    eol_descriptions = db.Column(JSON, nullable=True)  # EOL species descriptions (JSONB array)
    ...
    # EOL TraitBank Conservation Genetics (Research Integration)
    eol_population_genetics = db.Column(JSON, nullable=True)  # JSONB: Population genetics data
    eol_morphological_variation = db.Column(JSON, nullable=True)  # JSONB: Morphological variation
    eol_environmental_adaptation = db.Column(JSON, nullable=True)  # JSONB: Environmental adaptation
    eol_conservation_status = db.Column(JSON, nullable=True)  # JSONB: Conservation status details
    
    # GBIF Distribution & Regional Data (Research Integration)
    gbif_distribution = db.Column(JSON, nullable=True)  # JSONB: Distribution data from GBIF
    region_codes = db.Column(JSON, nullable=True)  # JSONB: ISO country/region codes
    
    # Media Provenance & Attribution (Research Integration)
    media_provenance = db.Column(JSON, nullable=True)  # JSONB: Media with full attribution
    
    # Data Quality & Analytics (Research Integration)
    trait_confidence = db.Column(JSON, nullable=True)  # JSONB: Confidence scores for each trait
    data_origin = db.Column(JSON, nullable=True)  # JSONB: Field -> source mapping
    conservation_priority_score = db.Column(db.Numeric, nullable=True)  # Conservation priority (0-100, NUMERIC)
    
    # Sync Timestamps (Research Integration)
    gbif_last_synced_at = db.Column(DateTime, nullable=True)  # Last GBIF sync
    eol_last_synced_at = db.Column(DateTime, nullable=True)  # Last EOL sync
    ai_last_synced_at = db.Column(DateTime, nullable=True)  # Last AI enrichment sync
    ethnobotany_last_updated = db.Column(DateTime, nullable=True)  # Last ethnobotany enrichment
    
    # Ethnobotany & Traditional Knowledge (Cultural Integration)
    ethnobotany_data = db.Column(JSON, nullable=True)  # JSONB: Traditional uses, indigenous names, cultural significance
    ...
    # Phase 1: Enhanced Visual Analysis Fields (zero-cost AI extraction)
    flower_color = db.Column(String(200), nullable=True)  # Comma-separated colors: "white, pink, purple"
    bloom_stage = db.Column(String(50), nullable=True)  # bud, open, past_bloom
    inflorescence_type = db.Column(String(100), nullable=True)  # raceme, panicle, spike, solitary
    inflorescence_position = db.Column(String(50), nullable=True)  # terminal, lateral, basal
    bloombot_category = db.Column(String(100), nullable=True)  # AI classification category
    widget_visibility = db.Column(Boolean, default=True)  # Show in widgets
    image_caption = db.Column(Text, nullable=True)  # AI-generated image caption
    
    # Phase 2: Advanced Morphology Analysis Fields (GPT-4o enhanced AI)
    leaf_shape = db.Column(String(100), nullable=True)  # lanceolate, ovate, terete, etc.
    pseudobulb_form = db.Column(String(100), nullable=True)  # ovoid, conical, cylindrical
    labellum_type = db.Column(String(100), nullable=True)  # simple, lobed, fringed
    flower_resupination = db.Column(Boolean, nullable=True)  # True if flower is resupinate
    keiki_formation = db.Column(String(50), nullable=True)  # frequent, occasional, rare, none
    rhizome_spread_type = db.Column(String(50), nullable=True)  # sympodial, monopodial
    leaf_venation = db.Column(String(50), nullable=True)  # parallel, reticulate
    tissue_succulence = db.Column(String(50), nullable=True)  # high, medium, low
    growth_rate = db.Column(String(50), nullable=True)  # fast, moderate, slow
    flower_longevity_days = db.Column(Integer, nullable=True)  # Days flower lasts
    dormant_leaf_drop = db.Column(Boolean, nullable=True)  # True if leaves drop during dormancy
    growth_eye_activation = db.Column(String(50), nullable=True)  # spring, fall, year_round
    
    # Phase 3: External API Integration Fields (GBIF, RHS, EOL)
    taxonomic_status = db.Column(String(50), nullable=True)  # GBIF: accepted, synonym, doubtful, misapplied
    taxonomic_authority = db.Column(String(300), nullable=True)  # Full citation authority (e.g., "Lindley 1840")
    continent = db.Column(String(100), nullable=True)  # Geographic continent from GBIF
    fragrance = db.Column(String(100), nullable=True)  # EOL: fragrant, unscented, variable, unknown
    fragrance_description = db.Column(Text, nullable=True)  # Detailed fragrance notes from EOL TraitBank
    parent_species_1 = db.Column(String(200), nullable=True)  # RHS: First parent species name
    parent_species_2 = db.Column(String(200), nullable=True)  # RHS: Second parent species name
    hybrid_formula = db.Column(String(500), nullable=True)  # RHS: Complete hybrid formula
...
class BotanistVisionResult(db.Model):
    """Results from Digital Botanist Vision AI analysis"""
    __tablename__ = 'botanist_vision_results'
    
    id = db.Column(Integer, primary_key=True)
    orchid_image_id = db.Column(Integer, nullable=True)
    image_url = db.Column(Text, nullable=True)
    blind_identification = db.Column(Boolean, default=True)
    
    # AI Identification
    ai_genus = db.Column(String(100), nullable=True)
    ai_species = db.Column(String(100), nullable=True)
    ai_confidence = db.Column(Float, nullable=True)
    identification_method = db.Column(Text, nullable=True)
    
    # Accuracy tracking
    correct_genus = db.Column(Boolean, nullable=True)
    correct_species = db.Column(Boolean, nullable=True)
    database_genus = db.Column(String(100), nullable=True)
    database_species = db.Column(String(100), nullable=True)
    identification_accuracy = db.Column(String(50), nullable=True)
    
    # Botanical characteristics
    sepal_count = db.Column(Integer, nullable=True)
    sepal_color = db.Column(Text, nullable=True)
    sepal_shape = db.Column(Text, nullable=True)
    petal_count = db.Column(Integer, nullable=True)
    petal_color = db.Column(Text, nullable=True)
    petal_shape = db.Column(Text, nullable=True)
    labellum_shape = db.Column(Text, nullable=True)
    labellum_color = db.Column(Text, nullable=True)
    column_description = db.Column(Text, nullable=True)
    
    # Full analysis
    vision_analysis = db.Column(Text, nullable=True)
    botanical_terms_used = db.Column(Text, nullable=True)
    scientific_name = db.Column(String(200), nullable=True)
    
    # AI provider info
    ai_provider = db.Column(String(50), nullable=True)
    processing_time = db.Column(Float, nullable=True)
    ...
    def __repr__(self):
        return f'<BotanistVisionResult {self.scientific_name or self.id}>'
...
# ============================================================================
# FEATURED ARTICLES SYSTEM - Content Management
# ============================================================================
...
class AssetPlacement(enum.Enum):
    """Where an asset appears in an article"""
    HERO = "hero"
    INLINE = "inline"
    GALLERY = "gallery"

class SubmissionStatus(enum.Enum):
    """Public submission workflow status"""
    RECEIVED = "received"
...
class Author(db.Model):
    ...
    avatar_asset_id = db.Column(Integer, db.ForeignKey('assets.id'), nullable=True)
    website_url = db.Column(String(500), nullable=True)
    created_at = db.Column(DateTime, default=datetime.utcnow)
    
    # Relationships
    articles = db.relationship('Article', back_populates='author', lazy='dynamic')
    avatar = db.relationship('Asset', foreign_keys=[avatar_asset_id])
    ...
    def to_dict(self):
        return {
            ...
            'email': self.email,
            'bio': self.bio,
            'avatar_url': self.avatar.storage_url if self.avatar else None,
            'website_url': self.website_url
        }

class Asset(db.Model):
    """Images and files for articles"""
    __tablename__ = 'assets'
    
    id = db.Column(Integer, primary_key=True)
    kind = db.Column(Enum(AssetKind), nullable=False, default=AssetKind.IMAGE)
    ...
    credit = db.Column(String(500), nullable=True)
    storage_url = db.Column(String(1000), nullable=False)
    created_at = db.Column(DateTime, default=datetime.utcnow)
...
class Article(db.Model):
    """Featured articles with markdown content"""
    __tablename__ = 'articles'
    
    id = db.Column(Integer, primary_key=True)
    slug = db.Column(String(200), unique=True, nullable=False, index=True)
    title = db.Column(String(500), nullable=False)
    subtitle = db.Column(String(500), nullable=True)
    excerpt = db.Column(Text, nullable=True)
    content_markdown = db.Column(Text, nullable=False)
    content_html = db.Column(Text, nullable=True)
    status = db.Column(Enum(ArticleStatus), nullable=False, default=ArticleStatus.DRAFT)
    featured = db.Column(Boolean, default=False, index=True)
    hero_image_id = db.Column(Integer, db.ForeignKey('assets.id'), nullable=True)
    author_id = db.Column(Integer, db.ForeignKey('authors.id'), nullable=False)
    tags = db.Column(JSON, nullable=True)
    youtube_url = db.Column(String(500), nullable=True)
    created_at = db.Column(DateTime, default=datetime.utcnow, index=True)
    updated_at = db.Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    published_at = db.Column(DateTime, nullable=True, index=True)
    
    # Relationships
    author = db.relationship('Author', back_populates='articles')
    hero_image = db.relationship('Asset', foreign_keys=[hero_image_id])
    article_assets = db.relationship('ArticleAsset', back_populates='article', lazy='dynamic', cascade='all, delete-orphan')
    ...
    def to_dict(self, include_content=False):
        ...
        if self.hero_image:
            data['hero_image'] = {
                'url': self.hero_image.storage_url,
                'alt': self.hero_image.alt_text or self.title
            }
    ...
    @staticmethod
    def generate_slug(title, existing_id=None):
        ...
        while True:
            ...
            if existing_id:
                query = query.filter(Article.id != existing_id)
            if not query.first():
                return slug
...
class ArticleAsset(db.Model):
    """Junction table linking articles to assets with placement info"""
    __tablename__ = 'article_assets'
    
    id = db.Column(Integer, primary_key=True)
    article_id = db.Column(Integer, db.ForeignKey('articles.id'), nullable=False)
    asset_id = db.Column(Integer, db.ForeignKey('assets.id'), nullable=False)
    placement = db.Column(Enum(AssetPlacement), nullable=False, default=AssetPlacement.INLINE)
    block_anchor = db.Column(String(500), nullable=True)
    caption = db.Column(Text, nullable=True)
    order_index = db.Column(Integer, default=0)
    
    # Relationships
    article = db.relationship('Article', back_populates='article_assets')
...
class Submission(db.Model):
    """Public article submissions for moderation"""
    __tablename__ = 'submissions'
    
    id = db.Column(Integer, primary_key=True)
    submitter_name = db.Column(String(200), nullable=False)
    submitter_email = db.Column(String(120), nullable=False)
    proposed_title = db.Column(String(500), nullable=False)
    proposed_excerpt = db.Column(Text, nullable=True)
    proposed_markdown = db.Column(Text, nullable=False)
    linked_asset_ids = db.Column(JSON, nullable=True)
    status = db.Column(Enum(SubmissionStatus), nullable=False, default=SubmissionStatus.RECEIVED)
    moderator_notes = db.Column(Text, nullable=True)
    created_at = db.Column(DateTime, default=datetime.utcnow, index=True)
    updated_at = db.Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    ...
    def to_dict(self):
        return {
            ...
            'linked_asset_ids': self.linked_asset_ids or [],
            'status': self.status.value,
...
class JuliusAIQuery(db.Model):
    """Track Julius AI queries to identify data enrichment opportunities"""
    __tablename__ = 'julius_ai_queries'
    
    id = db.Column(Integer, primary_key=True)
    endpoint = db.Column(String(200), nullable=False, index=True)
    query_params = db.Column(JSON, nullable=True)
    genus_queried = db.Column(String(100), nullable=True, index=True)
    species_queried = db.Column(String(100), nullable=True, index=True)
    data_requested = db.Column(String(100), nullable=True, index=True)
    ip_address = db.Column(String(50), nullable=True)
    user_agent = db.Column(String(500), nullable=True)
    response_status = db.Column(Integer, nullable=True)
    execution_time_ms = db.Column(Float, nullable=True)
    created_at = db.Column(DateTime, default=datetime.utcnow, index=True)
    ...
    def to_dict(self):
        return {
            ...
            'genus_queried': self.genus_queried,
...
class ResearchDocument(db.Model):
    """Catalog of research documents and academic PDFs"""
    __tablename__ = 'research_documents'
    
    id = db.Column(Integer, primary_key=True)
    title = db.Column(String(500), nullable=False, index=True)
    author = db.Column(String(300), nullable=False)
    year = db.Column(Integer, nullable=False, index=True)
    publisher = db.Column(String(200), nullable=True)
    doi = db.Column(String(200), nullable=True)
    isbn = db.Column(String(50), nullable=True)
    
    # File information
    file_path = db.Column(String(500), nullable=False)
    file_name = db.Column(String(300), nullable=False)
    file_size_mb = db.Column(Float, nullable=True)
    page_count = db.Column(Integer, nullable=True)
    
    # Document metadata
    document_type = db.Column(String(50), nullable=False, default='book')
    themes = db.Column(JSON, nullable=True)
    genera_covered = db.Column(JSON, nullable=True)
    language = db.Column(String(20), default='English')
    
    # Searchability
    abstract = db.Column(Text, nullable=True)
    keywords = db.Column(JSON, nullable=True)
    is_searchable = db.Column(Boolean, default=True)
    ...
    # Relationships
    topics = db.relationship('DocumentTopic', back_populates='document', cascade='all, delete-orphan')
    knowledge_cards = db.relationship('GenusKnowledgeCard', back_populates='document')
    ...
    def to_dict(self):
        return {
            ...
            'genera_covered': self.genera_covered or [],
...
class DocumentTopic(db.Model):
    """Searchable topics extracted from research documents"""
    __tablename__ = 'document_topics'
    
    id = db.Column(Integer, primary_key=True)
    document_id = db.Column(Integer, db.ForeignKey('research_documents.id'), nullable=False, index=True)
    
    # Topic classification
    topic_type = db.Column(String(50), nullable=False, index=True)
    topic_name = db.Column(String(200), nullable=False, index=True)
    
    # Topic details
    description = db.Column(Text, nullable=True)
    page_references = db.Column(JSON, nullable=True)
    
    # Context
    cultural_area = db.Column(String(100), nullable=True, index=True)
    genus = db.Column(String(100), nullable=True, index=True)
    
    # Searchability
    search_tags = db.Column(JSON, nullable=True)
    relevance_score = db.Column(Float, default=1.0)
    
    created_at = db.Column(DateTime, default=datetime.utcnow)
    
    # Relationship
    document = db.relationship('ResearchDocument', back_populates='topics')
    ...
    def to_dict(self):
        return {
            ...
            'description': self.description,
            'page_references': self.page_references or [],
            'cultural_area': self.cultural_area,
            'genus': self.genus,
...
class GenusKnowledgeCard(db.Model):
    """Genus-specific knowledge cards linked to research documents"""
    __tablename__ = 'genus_knowledge_cards'
    ...
    medicinal_uses = db.Column(JSON, nullable=True)
    traditional_uses = db.Column(JSON, nullable=True)
    
    # Chemical compounds
    active_compounds = db.Column(JSON, nullable=True)
    compound_classes = db.Column(JSON, nullable=True)
    
    # Cultural context
    cultural_areas = db.Column(JSON, nullable=True)
    indigenous_names = db.Column(JSON, nullable=True)
    
    # Research data
    pharmacological_effects = db.Column(JSON, nullable=True)
    clinical_studies = db.Column(JSON, nullable=True)
    
    # Reference information
    page_references = db.Column(JSON, nullable=True)
    key_findings = db.Column(Text, nullable=True)
    
    # Conservation
    conservation_status = db.Column(String(50), nullable=True)
    conservation_notes = db.Column(Text, nullable=True)
    
    # Metadata
    confidence_score = db.Column(Float, default=0.8)
    last_verified = db.Column(DateTime, default=datetime.utcnow)
    created_at = db.Column(DateTime, default=datetime.utcnow)
    updated_at = db.Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    
    # Relationships
    document = db.relationship('ResearchDocument', back_populates='knowledge_cards')
    ...
    def to_dict(self):
        return {
            ...
            'genus': self.genus,
            'document_id': self.document_id,
            'medicinal_uses': self.medicinal_uses or [],
            'traditional_uses': self.traditional_uses or [],
            'active_compounds': self.active_compounds or {},
            'compound_classes': self.compound_classes or [],
            'cultural_areas': self.cultural_areas or [],
            'indigenous_names': self.indigenous_names or {},
            'pharmacological_effects': self.pharmacological_effects or [],
            'clinical_studies': self.clinical_studies or [],
            'page_references': self.page_references or [],
...
class OrchidImageReport(db.Model):
    """
    User-submitted reports about orchid image/name mismatches
    Critical for data integrity and organizational credibility
    """
    __tablename__ = 'orchid_image_reports'
    
    id = db.Column(Integer, primary_key=True)
    orchid_id = db.Column(Integer, db.ForeignKey('orchid_record.id'), nullable=False)
    
    # Report details
    issue_type = db.Column(String(50), nullable=False)
    description = db.Column(Text, nullable=False)
    reported_by_email = db.Column(String(120), nullable=True)
    reported_by_name = db.Column(String(100), nullable=True)
    
    # Diagnostic information
    current_scientific_name = db.Column(String(200), nullable=True)
    suggested_name = db.Column(String(200), nullable=True)
    image_source = db.Column(String(100), nullable=True)
    image_url = db.Column(String(500), nullable=True)
    
    # Status tracking
    status = db.Column(String(20), default='pending')
    priority = db.Column(String(20), default='normal')
    resolution_notes = db.Column(Text, nullable=True)
    resolved_by = db.Column(String(120), nullable=True)
    ...
    # Relationship
    orchid = db.relationship('OrchidRecord', backref='image_reports')
    ...
    def to_dict(self):
        return {
            ...
            'issue_type': self.issue_type,
            'description': self.description,
            'reported_by_email': self.reported_by_email,
            'reported_by_name': self.reported_by_name,
            'current_scientific_name': self.current_scientific_name,
...
# ============================================================================
# ORCHID UNIVERSITY CURRICULUM SYSTEM
# ============================================================================

class OCUCourse(db.Model):
    """Orchid Continuum University courses"""
    __tablename__ = 'ocu_courses'
    
    id = db.Column(Integer, primary_key=True)
    course_code = db.Column(String(20), unique=True, nullable=False)  # C1, C2, etc.
    title = db.Column(String(200), nullable=False)
    description = db.Column(Text)
    order_num = db.Column(Integer, default=0)
    estimated_hours = db.Column(Integer)
    difficulty_level = db.Column(String(50))  # beginner, intermediate, advanced
    companion_character = db.Column(String(100))  # Sprig, Buzz, Mica Myco, etc.
    is_published = db.Column(Boolean, default=False)
    created_at = db.Column(DateTime, default=datetime.utcnow)
    updated_at = db.Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    
    # Relationships
    lessons = db.relationship('OCULesson', backref='course', lazy='dynamic', cascade='all, delete-orphan')
    ...
    def to_dict(self):
        return {
            ...
            'difficulty_level': self.difficulty_level,
            'companion_character': self.companion_character,
...
class OCULesson(db.Model):
    """Individual lessons within courses"""
    __tablename__ = 'ocu_lessons'
    
    id = db.Column(Integer, primary_key=True)
    course_id = db.Column(Integer, db.ForeignKey('ocu_courses.id'), nullable=False)
    lesson_code = db.Column(String(20), unique=True, nullable=False)  # C1L1, C1L2, etc.
    title = db.Column(String(200), nullable=False)
    content_markdown = db.Column(Text)  # Full lesson content in Markdown
    summary = db.Column(Text)
    order_num = db.Column(Integer, default=0)
    estimated_time_minutes = db.Column(Integer)
    
    # Content sources and gaps (from Julius's research)
    content_sources = db.Column(JSON)  # Links to external resources
    content_gaps = db.Column(JSON)  # Missing diagrams, text, etc.
    required_diagrams = db.Column(JSON)  # List of Canva prompts needed
    
    # Status
    is_published = db.Column(Boolean, default=False)
    completion_required = db.Column(Boolean, default=True)
    ...
    def to_dict(self):
        return {
            ...
            'content_sources': self.content_sources or [],
            'content_gaps': self.content_gaps or [],
            'required_diagrams': self.required_diagrams or [],
            'is_published': self.is_published,
            'created_at': self.created_at.isoformat() if self.created_at else None
        }
...
class OCUGlossaryTerm(db.Model):
    """Glossary terms for curriculum"""
    __tablename__ = 'ocu_glossary_terms'
    
    id = db.Column(Integer, primary_key=True)
    term = db.Column(String(200), unique=True, nullable=False)
    definition = db.Column(Text, nullable=False)
    category = db.Column(String(100))  # taxonomy, conservation, ecology, etc.
    related_terms = db.Column(JSON)  # List of related term IDs
    examples = db.Column(JSON)  # List of example species/uses
    view_count = db.Column(Integer, default=0)
    image_path = db.Column(String(500))  # Path to illustration image
    etymology = db.Column(Text)  # Greek/Latin root words and meaning
    pronunciation = db.Column(String(200))  # Phonetic pronunciation guide
    created_at = db.Column(DateTime, default=datetime.utcnow)
    updated_at = db.Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class OCUUserProgress(db.Model):
    """Track user progress through curriculum"""
    __tablename__ = 'ocu_user_progress'
    
    id = db.Column(Integer, primary_key=True)
    user_email = db.Column(String(120), nullable=False)  # Email-based for FCOS integration
    lesson_id = db.Column(Integer, db.ForeignKey('ocu_lessons.id'), nullable=False)
    course_id = db.Column(Integer, db.ForeignKey('ocu_courses.id'), nullable=False)
    ...
    # Engagement metrics
    selected_companion = db.Column(String(100))  # Which companion they chose
    scroll_percentage = db.Column(Integer, default=0)
    completed = db.Column(Boolean, default=False)
    
    # Relationships
    lesson = db.relationship('OCULesson', backref='progress_records')
    course = db.relationship('OCUCourse', backref='progress_records')
    
    __table_args__ = (
        db.UniqueConstraint('user_email', 'lesson_id', name='unique_user_lesson'),
    )
...
class OCUQuizAttempt(db.Model):
    """Quiz attempts for lessons"""
    __tablename__ = 'ocu_quiz_attempts'
    
    id = db.Column(Integer, primary_key=True)
    user_email = db.Column(String(120), nullable=False)
    lesson_id = db.Column(Integer, db.ForeignKey('ocu_lessons.id'), nullable=False)
    score = db.Column(Float)  # 0-100
    answers = db.Column(JSON)  # User's answers
    passed = db.Column(Boolean, default=False)
    attempt_number = db.Column(Integer, default=1)
    created_at = db.Column(DateTime, default=datetime.utcnow)
    
    lesson = db.relationship('OCULesson', backref='quiz_attempts')
...
class OCUCertificate(db.Model):
    """Course completion certificates"""
    __tablename__ = 'ocu_certificates'
    
    id = db.Column(Integer, primary_key=True)
    user_email = db.Column(String(120), nullable=False)
    user_name = db.Column(String(200), nullable=False)
    course_id = db.Column(Integer, db.ForeignKey('ocu_courses.id'), nullable=False)
    certificate_code = db.Column(String(50), unique=True, nullable=False)
    issued_date = db.Column(DateTime, default=datetime.utcnow)
    
    # Certificate data
    total_lessons = db.Column(Integer)
    total_time_hours = db.Column(Float)
    average_quiz_score = db.Column(Float)
    
    course = db.relationship('OCUCourse', backref='certificates')
...
class GenusAbbreviation(db.Model):
    """Genus abbreviation patterns (from Julius's dataset)"""
    __tablename__ = 'genus_abbreviations'
    
    id = db.Column(Integer, primary_key=True)
    abbreviation = db.Column(String(20), nullable=False)
    full_genus = db.Column(String(200), nullable=False)
    pattern = db.Column(String(50))  # first_letter, first_two, first_three, first_four
    created_at = db.Column(DateTime, default=datetime.utcnow)
    
    __table_args__ = (
        db.UniqueConstraint('abbreviation', 'full_genus', name='unique_abbrev_genus'),
    )
    ...
    def to_dict(self):
        return {
            ...
            'full_genus': self.full_genus,
            'pattern': self.pattern
        }


# ============================================================================
# BLOOMBUILDER: Interactive Orchid Morphology Lab
# ============================================================================

class BloomBuilderSpecies(db.Model):
    """Species data for BloomBuilder morphology annotation tool"""
    __tablename__ = 'bloombuilder_species'
    
    id = db.Column(Integer, primary_key=True)
    species = db.Column(String(200), nullable=False, unique=True)
    genus = db.Column(String(100), nullable=False)
    family = db.Column(String(100), default='Orchidaceae')
    
    # Image URLs
    herbarium_url = db.Column(String(500), nullable=True)
    photo_url = db.Column(String(500), nullable=True)
    diagram_url = db.Column(String(500), nullable=True)
    
    # Citation and source
    source_reference = db.Column(Text, nullable=True)
    
    # Morphology profile type
    profile_type = db.Column(String(50), default='default_orchid')  # default_orchid, pouch_orchid, spurred_orchid, mimic_orchid, vine_orchid
    
    # Ecological data
    habitat = db.Column(Text, nullable=True)
    distribution = db.Column(String(500), nullable=True)
    pollinators = db.Column(String(500), nullable=True)
    ecological_notes = db.Column(Text, nullable=True)
    conservation_status = db.Column(String(200), nullable=True)
    evolutionary_notes = db.Column(Text, nullable=True)
    external_links = db.Column(Text, nullable=True)
    ...
    def to_dict(self):
        return {
            ...
            'genus': self.genus,
            'family': self.family,
            'herbarium_url': self.herbarium_url,
            'photo_url': self.photo_url,
            ...
            'ecological_notes': self.ecological_notes,
            'conservation_status': self.conservation_status,
            'evolutionary_notes': self.evolutionary_notes,
...
class BloomBuilderAnnotation(db.Model):
    """User annotations of orchid parts on herbarium images"""
    __tablename__ = 'bloombuilder_annotations'
    
    id = db.Column(Integer, primary_key=True)
    species_id = db.Column(Integer, db.ForeignKey('bloombuilder_species.id'), nullable=False)
    user_id = db.Column(String, db.ForeignKey('users.id'), nullable=True)
    session_id = db.Column(String(100), nullable=True)  # For anonymous users
    
    # Annotation data
    part_name = db.Column(String(100), nullable=False)  # dorsal_sepal, labellum, column, etc.
    box_data = db.Column(JSON, nullable=False)  # {x, y, width, height}
    image_type = db.Column(String(20), default='herbarium')  # herbarium, photo, diagram
    
    # Validation tracking
    agrees = db.Column(Integer, default=0)
    suggestions = db.Column(Integer, default=0)
    ...
    # Relationships
    species = db.relationship('BloomBuilderSpecies', backref='annotations')
...
class BloomBuilderValidation(db.Model):
    """Community validation votes for annotations"""
    __tablename__ = 'bloombuilder_validations'
    
    id = db.Column(Integer, primary_key=True)
    annotation_id = db.Column(Integer, db.ForeignKey('bloombuilder_annotations.id'), nullable=False)
    user_id = db.Column(String, db.ForeignKey('users.id'), nullable=True)
    session_id = db.Column(String(100), nullable=True)
    
    validation_type = db.Column(String(20), nullable=False)  # 'agree', 'suggest_fix'
    suggestion_notes = db.Column(Text, nullable=True)
    suggested_box_data = db.Column(JSON, nullable=True)
    
    created_at = db.Column(DateTime, default=datetime.utcnow)
    
    # Relationships
    annotation = db.relationship('BloomBuilderAnnotation', backref='validations')
    
    __table_args__ = (
        db.UniqueConstraint('annotation_id', 'user_id', 'session_id', name='unique_user_annotation_validation'),
    )
    ...
    def to_dict(self):
        return {
            ...
            'suggestion_notes': self.suggestion_notes,
            'suggested_box_data': self.suggested_box_data,
            'created_at': self.created_at.isoformat()
        }
