...
from models import OrchidRecord, GenusKnowledgeCard, ResearchDocument
...
class ValidatedOrchidOfDay:
    ...
    def get_enhanced_orchid_of_day(self):
        ...
        try:
            ...
            enhanced_content = {
                ...
                'medicinal_knowledge': self._get_medicinal_knowledge(orchid)  # NEW: Auto-fetch from Research Library
            }
    ...
    def _get_medicinal_knowledge(self, orchid):
        """
        Auto-fetch medicinal knowledge from Research Library based on orchid genus
        Returns medicinal/traditional knowledge if available
        """
        if not orchid.genus:
            return None
        
        try:
            # Query genus knowledge cards for this orchid's genus (case-insensitive)
            knowledge_cards = GenusKnowledgeCard.query.filter(
                func.lower(GenusKnowledgeCard.genus) == func.lower(orchid.genus)
            ).all()
            
            if not knowledge_cards:
                return None
            
            # Compile knowledge from all cards
            medicinal_info = {
                'has_knowledge': True,
                'genus': orchid.genus,
                'traditional_uses': [],
            ...
            for card in knowledge_cards:
                ...
                # Compile traditional uses
                if card.traditional_uses:
                    for use in card.traditional_uses:
                        if use and use not in medicinal_info['traditional_uses']:
                            medicinal_info['traditional_uses'].append(use)
                
                # Compile medicinal uses
                if card.medicinal_uses:
                    for use in card.medicinal_uses:
                        if use and use not in medicinal_info['medicinal_uses']:
                            medicinal_info['medicinal_uses'].append(use)
                
                # Compile active compounds
                if card.active_compounds:
                    for compound in card.active_compounds:
                        if compound and compound not in medicinal_info['active_compounds']:
                            medicinal_info['active_compounds'].append(compound)
                
                # Compile cultural areas
                if card.cultural_areas:
                    for area in card.cultural_areas:
                        if area and area not in medicinal_info['cultural_areas']:
                            medicinal_info['cultural_areas'].append(area)
                
                # Add source citation
                if document:
                    source_citation = f"{document.title}"
                    if document.author:
                        source_citation += f" ({document.author}, {document.year})"
                    elif document.year:
                        source_citation += f" ({document.year})"
                    
                    if source_citation not in medicinal_info['sources']:
                        medicinal_info['sources'].append(source_citation)
                    
                    # Add page references if available
                    if card.page_references:
                        medicinal_info['page_references'] = card.page_references
                
                # Add key findings
                if card.key_findings:
                    if 'key_findings' not in medicinal_info:
                        medicinal_info['key_findings'] = []
                    medicinal_info['key_findings'].append(card.key_findings)
            
            # Add disclaimer
            medicinal_info['disclaimer'] = (
                "⚠️ Educational/historical purposes only - NOT medical advice. "
                "Never harvest wild orchids. Consult healthcare professionals before using any plant medicinally."
            )
            
            return medicinal_info if medicinal_info['traditional_uses'] or medicinal_info['medicinal_uses'] else None
            
        except Exception as e:
            logger.error(f"Error fetching medicinal knowledge for {orchid.genus}: {e}")
            return None
