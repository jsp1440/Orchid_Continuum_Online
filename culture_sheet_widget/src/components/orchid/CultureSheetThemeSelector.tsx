...
import { cultureSheetThemes } from '../../data/themeConfigs';
...
export const CultureSheetThemeSelector: React.FC<Props> = ({ currentTheme, onThemeChange }) => {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {(Object.keys(cultureSheetThemes) as SheetTheme[]).map(theme => {
        const config = cultureSheetThemes[theme];
        return (
          <button
            key={theme}
