...
def api_crossword_puzzle():
    ...
    try:
        puzzle_data = generate_crossword_api()
...
def get_orchid_trivia(orchid_id):
    """Get trivia facts for a specific orchid"""
    orchid = OrchidRecord.query.get(orchid_id)
    
    if not orchid:
        return jsonify({'error': 'Orchid not found'}), 404
    
    # Build trivia facts from available data
    facts = []
    
    if orchid.genus:
        facts.append(f"Genus: {orchid.genus}")
    
    if orchid.native_habitat:
        facts.append(f"Habitat: {orchid.native_habitat}")
    
    if orchid.bloom_time:
        facts.append(f"Blooms: {orchid.bloom_time}")
    
    if orchid.growth_habit:
        facts.append(f"Growth: {orchid.growth_habit}")
    
    if orchid.climate_preference:
        facts.append(f"Climate: {orchid.climate_preference}")
    
    if orchid.cultural_notes and len(orchid.cultural_notes) > 10:
        facts.append(orchid.cultural_notes)
    
    # If no facts, add a generic one
    if not facts:
        facts.append(f"{orchid.display_name or orchid.scientific_name} is a beautiful orchid species")
...
@games_bp.route('/api/rebus-puzzle')
def get_rebus_puzzle():
    """Get a random rebus puzzle for the memory match game"""
    # Simple rebus puzzles about orchids
    puzzles = [
        {
            'image_url': '/static/images/rebus/orchid-puzzle-1.png',
            'answer': 'ORCHID LOVER',
            'hint': 'Someone who loves orchids'
        },
        {
            'image_url': '/static/images/rebus/orchid-puzzle-2.png',
            'answer': 'FLOWER POWER',
            'hint': 'The strength of blooms'
        },
        {
            'image_url': '/static/images/rebus/orchid-puzzle-3.png',
            'answer': 'GROWING WILD',
            'hint': 'Natural orchid state'
        }
    ]
