    ...
    def initialize_services(self):
        ...
        try:
            ...
            if service_account_json:
                ...
                base_credentials = Credentials.from_service_account_info(
                    credentials_info, 
                    scopes=self.scopes
                )
                
                # Use service account credentials
                # Note: Service accounts have 15GB quota limit
                # For larger storage needs, files should be uploaded to shared drives
                # or use OAuth user credentials instead
                self.credentials = base_credentials
                logger.info("✅ Using service account credentials")
                logger.info("⚠️ Note: Service account has 15GB storage limit")
