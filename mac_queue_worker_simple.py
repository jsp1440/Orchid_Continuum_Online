...
def lease(n=BATCH_SIZE):
    ...
    try:
        ...
        sql = """UPDATE harvest_jobs SET status='leased', lease_owner=%s, leased_at=NOW() 
                 WHERE id IN (SELECT id FROM harvest_jobs WHERE status='pending' 
                 ORDER BY priority DESC LIMIT %s FOR UPDATE SKIP LOCKED) 
...
def save(img, tid):
    ...
    try:
        ...
        if r.fetchone():
            return False
        sql = """INSERT INTO orchid_images (taxonomy_id, image_url, image_source, image_type, 
                 country, latitude, longitude, observation_date, year_observed, 
                 gbif_occurrence_key, created_at, updated_at) 
                 VALUES (%s, %s, 'GBIF', 'observation', %s, %s, %s, %s, %s, %s, NOW(), NOW())"""
        r.execute(sql, (tid, img['url'], img.get('country'), img.get('lat'), img.get('lon'), 
                       img.get('date'), img.get('year'), img.get('key')))
        c.commit()
        return True
    except:
        c.rollback()
