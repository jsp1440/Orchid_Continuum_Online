...
# Admin credentials from environment variables - fail closed if not set
...
if not ADMIN_PASSWORD:
    ...
    if ADMIN_EMAIL:
        logger.info(f"✅ Admin authentication properly configured for {ADMIN_EMAIL}")
    else:
        logger.info("✅ Admin authentication properly configured (password-only mode)")
...
def admin_login():
    ...
    # Check if admin access is disabled due to missing environment variables
    ...
    if request.method == 'POST':
        ...
        # Validate credentials - email is optional
        password_valid = password and check_password_hash(ADMIN_PASSWORD_HASH, password)
        # Email is truly optional - if provided, it must match (case-insensitive); if not provided, it's still valid
        email_valid = not email or not ADMIN_EMAIL or (email.lower() == ADMIN_EMAIL.lower())
        
        if password_valid and email_valid:
