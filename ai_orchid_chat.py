...
from research_lookup_utility import (
    get_genus_research_data, 
    get_research_context_for_ai,
    check_genus_has_research,
    increment_research_view_count
)
...
- Accessing ethnobotanical and medicinal research from academic sources (when available for specific genera)

IMPORTANT: When research data is provided in the context, you may share ethnobotanical and traditional knowledge for educational purposes. Always emphasize:
1. This information is for EDUCATIONAL purposes only
2. This is NOT medical advice
3. Never harvest wild orchids
4. Always consult healthcare professionals before using any plant medicinally
        ...
        try:
            ...
            for genus in orchid_genera:
                if genus in query_lower:
                    ...
                    # Check for research data on this genus
                    if check_genus_has_research(genus):
                        research_context = get_research_context_for_ai(genus)
                        if research_context:
                            context['research_data'].append({
                                'genus': genus.capitalize(),
                                'context': research_context
                            })
                            # Increment view count
                            increment_research_view_count(genus)
