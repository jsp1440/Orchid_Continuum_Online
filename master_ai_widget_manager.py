...
class MasterAIWidgetManager:
    ...
    def start_monitoring(self):
        """Start background monitoring systems - GUARDED by AI kill-switch"""
        # PRODUCTION STABILITY: Don't start if AI is disabled
        from app.settings import ORCHID_AI_ENABLED
        
        if not ORCHID_AI_ENABLED:
            logger.info("🔒 AI Widget Manager monitoring DISABLED (ORCHID_AI_ENABLED=false)")
            logger.info("   Scheduled AI tasks will NOT run - protects OpenAI quota!")
            return
        ...
        logger.info("✅ AI Widget Manager monitoring systems started (AI enabled)")
